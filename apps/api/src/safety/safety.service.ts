...
export class SafetyService {
  ...
  report(reporterId: string, dto: ReportDto) {
    ...
    return this.prisma.report.create({
      data: {
        ...
        reason: dto.reason,
        details: dto.details,
        evidence: dto.evidence as never,
      },
    });
  }

  reportQueue(status?: ReportStatus) {
    return this.prisma.report.findMany({
      where: status ? { status } : {},
      include: {
        reporter: { select: { id: true, profile: { select: { displayName: true } } } },
        reported: {
          select: { id: true, status: true, profile: { include: { photos: true } } },
        },
      },
  ...
  async resolve(reportId: string, reviewerId: string, dto: ResolveReportDto) {
    ...
    return this.prisma.$transaction(async (transaction) => {
      if (dto.userStatus && dto.userStatus !== UserStatus.DELETED) {
      ...
      await transaction.auditLog.create({
        data: {
          ...
          targetType: 'Report',
          targetId: reportId,
          metadata: { status: dto.status, userStatus: dto.userStatus },
        },
      });
      return updated;
    });
